...
# # select_project_all()の改良版
# def select_project_db() -> List[schemas.Project]:
#     with get_context_db() as db: # get_context_db()でsessionを取得
#         return db.query(models.Project).all()
...
def select_project_by_id(
    ...
    return bd.query(models.Project).filter(models.Project.project_id == project_id).first()

def select_project_by_name(
    db: Session,
    project_name: str,
) -> schemas.Project:
    return bd.query(models.Project).filter(models.Project.project_name == project_name).first()
...
def add_project(
    ...
    if exists:
        ...
        if commit:
            db.commit()
            db.refresh(data) # データベースの最新情報でオブジェクトを更新する
        return data
...
def select_experiment_by_project_id(
    ...
    return (
        ...
        .filter(models.Experiment.model_id == models.Model.model_id) # models.Experiment.model_id が models.Model.model_id を参照する外部キー（Foreign Key）であるため
        .all()
    )
