...
def start_run(
    ...
    device = torch.device(
        "cuda:0" if torch.cuda_is_availabe() else "cpu",
    )
    ...
    train_dataset = Cifar10Dataset(
        data_direcotry=os.path.join(upstream_directory, "train"),
        transform=transform
    )
    ...
    test_dataset = Cifar10Dataset(
        data_direcotry=os.path.join(upstream_directory, "test"),
        transform=transform
    )
    ...
    # onnx形式でモデルをエクスポート（TensorFlowなど他のフレームワークでも使用可能になる）
    ...
    # 学習済みモデルをMLflowにアップロード
    mlflow.log_artifact(model_file_name)
    mlflow.log_artifact(onnx_file_name)
    # tensorboardのログをMLflowに記録することで、後で可視化が可能になる
